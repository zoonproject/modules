...
GenerateCovariateReport <- function(.model, .ras, type = "DR"){
  ...
  GenerateDataReport <- function(input_data, output_file = "data_report.html", output_dir = report_dir, ...) {
    ## Get argument list
  ...
  GenerateRasterReport <- function(input_data, output_file = "raster_report.html", output_dir = report_dir, ...) {
    ## Get argument list
    args <- as.list(match.call())
    ## Get directory of report markdown template
  ...
  unlink(Sys.glob("rmarkdown-*"), force = TRUE, recursive = TRUE)
