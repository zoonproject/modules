#' @title Output module: ResponseCurve
#'
...
#' @section Version: 1.1
#' @section Date submitted: 2017-04-11
#'
#' @name ResponseCurve
#' @family output
ResponseCurve <- function(.model, .ras, cov = NULL) {
    ...
    # make a copy where the value is held for all others, and one for
    # predictions
    Eres <- Emeds <- Epred
    ...
    # ~~~~~~~~~~~~~~~~~~~~ plotting save default, for resetting...
    ...
    par(mar = c(0, 1, 1, 1))
    plot(-99, -99, xlim = c(1, 100), ylim = c(1, 100), cex = 0, main = "", 
    ...
    text(85, 50, labels = "prediction", col = coltits, cex = cexlab3, srt = 90, 
        font = 3)
    ...
    text(85, 50, labels = "denisty", col = coltits, cex = cexlab3, srt = 90, 
        font = 3)
    
    # PLOT number 4 for lay
    hist(covar[pres_idx], breaks = 50, xlim = c(covar_min, covar_max), axes = FALSE, 
        main = "", col = colpres, border = FALSE)
    axis(1, at = seq(covar_min, covar_max, by = 10), labels = FALSE, col = colaxes)
    axis(2, col = colaxes, las = 2)
    axis(3, at = seq(covar_min, covar_max, by = 10), labels = FALSE, tck = -0.25, 
        col = colaxes)
    box(col = colaxes)
    ...
    text(1, 75, labels = "Data", col = coltits2, adj = 0, cex = cexlab)
    text(85, 50, labels = "density", col = coltits, cex = cexlab3, srt = 90, 
        font = 3)
    
    # PLOT number 6 for lay
    hist(covar[back_idx], breaks = 50, xlim = c(covar_min, covar_max), axes = FALSE, 
        main = "", xlab = "", ylab = "F", las = 2, col = colpseu, border = FALSE)
    axis(1, at = seq(covar_min, covar_max, by = 10), las = 3, col = colaxes)
    axis(2, col = colaxes, las = 2)
    ...
    plot(-99, -99, xlim = c(1, 100), ylim = c(1, 100), cex = 0, main = "", 
        axes = FALSE, xlab = "", ylab = "")
    text(1, 90, labels = c(name), col = coltits, adj = 0, cex = cexlab2, font = 2)
